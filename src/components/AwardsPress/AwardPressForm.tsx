...
const AwardPressForm = ({addFilmInfo, postAwardsPress, setIsEditting, isEditting, epk_id}: IAwardPressForm) => {
  ...
  const submitAndClear = (endpoint: string, newItem: any) => {
    if ((newAward.award.name && newAward.award.year) || (newPress.press.publication && newPress.press.link)) {
    setPublication('')
    setLink('')
    setQuote('')
    setAwardName('') 
      ...
      <FormControl sx={{ m: 1, minWidth: 120 }}>
       ...
       <Select
        ...
        {select === 'award' && <>
          ...
          <TextField style={{ marginRight: '3%'}} id="outlined-basic" name="year" label="year" variant="outlined" value={awardYear} onChange={e => setAwardYear(e.target.value)} helperText="year required" required/>
          ...
          onClick={() => submitAndClear('awards', newAward)}>save</Button>
        </>}
        ...
        {select === 'press' && <>
          ...
          <TextField style={{ marginRight: '3%'}} id="outlined-basic" name="link" label="link" variant="outlined" value={link} onChange={e => setLink(e.target.value)} helperText="link required" required />
          <TextField style={{ marginRight: '3%'}} id="outlined-basic" name="description" label="description/quote" variant="outlined" value={quote} onChange={e => setQuote(e.target.value)}/>
          <Button style={{ background: '#ec5f27', height:'57px', marginRight: '3%'}} variant="text"
          onClick={() => submitAndClear('presses', newPress)}>save</Button>
        </>}
